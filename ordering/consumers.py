...
class KitchenConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if data.get('type') == 'status_update':
            await self.update_order_status(data['order_id'], data['status'])
...
class OrderConsumer(AsyncWebsocketConsumer):
    """Customer-facing consumer: subscribes to a single order's real-time status updates."""
    ...
    async def receive(self, text_data):
        pass  # customers don't send messages
